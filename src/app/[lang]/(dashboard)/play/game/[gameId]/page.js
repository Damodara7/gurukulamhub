...
export default function QuizPage({ params }) {
  ...
  const [quizId,setQuizId] = useState("venkat");
  const [gameId,setGameId] = useState(params.gameId);
  const [gamePin,setGamePin] = useState(null);
  const [gameTemplateId, setGameTemplateId] = useState(null);
  ...
  console.log("params", username, params.gameId)
  ...
  useEffect(() => {
    ...
    console.log("Socket.id, quizid, username...", socket.id, quizId, gameId, username)
  ...
  function createGame() {
    createTheGame("venkat",ramayana,  username);
  }

  function createTheGame(quizId, gameTemplateId, gameId, gamePin, username, startTime) {
    if (!startTime) startTime = new Date().getTime();
    console.log("Creating game..", roomId, quizId, username, startTime)
    // Create a new game and get the generated PIN
    socket.emit('createGame', { roomId,quizId, username, startTime });
  }
  ...
  function startGame() {
    ...
    socket.emit('startGame', { pin, quizId, username });
  }
