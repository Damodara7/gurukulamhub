...
import GeoRole from './geo-role.model.js' // Import your Role model
import { validateRoleCreateRequestDto, validateRoleUpdateRequestDto } from './geo-role.validator.js' // Import your DTO schema
...
export async function getById({ id }) {
  await connectMongo()
  try {
    const role = await GeoRole.findById({ _id: id, isDeleted: false })
    if (!role) {
...
export async function getAll() {
  await connectMongo()
  try {
    const roles = await GeoRole.find({ isDeleted: false }).sort({ createdAt: -1 }) // Sort by createdAt descending
...
export async function updateOne({ id, data }) {
  ...
  await connectMongo()
  try {
    const existingRole = await GeoRole.findOne({ _id: id, isDeleted: false })
    if (!existingRole) {
      return { status: 'error', message: 'Role not found or already deleted', result: null }
    }

    const updatedRole = await GeoRole.findByIdAndUpdate(id, data, { new: true })
    if (!updatedRole) {
      return { status: 'error', message: 'Role not found', result: null }
    }
