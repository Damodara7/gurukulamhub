...
export async function add({ data }) {
    ...
    try {
        ...
        const newRole = new GeoRole({ ...data });
        await newRole.save();
...
export async function getById({ id }) {
    await connectMongo();
    try {
        const role = await GeoRole.findById(id);
        if (!role) {
...
export async function getAll() {
    await connectMongo();
    try {
        const roles = await GeoRole.find().sort({ createdAt: -1 }); // Sort by createdAt descending
...
export async function deleteOne({ id }) {
    await connectMongo();
    try {
        const deletedRole = await GeoRole.findByIdAndDelete(id);
        if (!deletedRole) {
            return { status: 'error', message: 'Role not found' };
        }
