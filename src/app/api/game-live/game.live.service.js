...
import GamesLive from './game.live.model.js'
import { gameCreateRequestDtoSchema } from './game.live.validator.js'
import * as ApiResponseUtils from '@/utils/apiResponses'
import mongoose from 'mongoose'

const Artifact = 'GamesLive'
const ArtifactModel = GamesLive
...
export async function updateWithGamePin(gamePin, updateData) {
  await connectMongo()
  try {
...
export async function getById(id) {
  await connectMongo()
  try {
    const res = await ArtifactModel.find({})
    console.log({ res })
    const artifact = await ArtifactModel.findOne({ _id: mongoose.Schema.ObjectId('680f2ee57dbfa03048a97d47') })
    if (!artifact) {
      console.log(`No ${Artifact} found.`)
      const finalResult = {
        status: 'error',
        result: null,
        message: `${Artifact}(${id}) failed to get.`
      }
      return finalResult
    } else {
      const finalResult = {
        status: 'success',
        result: artifact,
        message: `${Artifact}(${id}) retrieved Successfully`
      }
      return finalResult
    }
  } catch (err) {
...
export async function getByGamePin(reqBody) {
