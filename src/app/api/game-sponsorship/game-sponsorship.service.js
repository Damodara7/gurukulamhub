...
export async function updateGameWithSponsorship(sponsorshipId, gameId, rewardId, allocatedAmount) {
  try {
    ...
    const reward = game.rewards.find(
    ...
    // Save both documents
    await Promise.all([game.save({ validateBeforeSave: false }), sponsorship.save()])
    ...
    const allRewardsFullySponsored = game.rewards.every(reward => {
      ...
      const totalAllocated =
        reward.sponsors?.reduce(
          (sum, sponsor) => sum + (sponsor.allocated || sponsor.rewardDetails?.allocated || 0),
          0
        ) || 0
