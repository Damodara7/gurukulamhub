...
import GameStatus from './game-status.model.js'
import { gameStatusCreateRequestDtoSchema } from './game-status.validator.js'
import * as ApiResponseUtils from '@/utils/apiResponses'

const Artifact = 'GameStatus'
const ArtifactModel = GameStatus
const CreateRequestDtoSchema = gameStatusCreateRequestDtoSchema
...
export async function getByGamePin(gamePin) {
  await connectMongo()
  try {
    const allArtifacts = await ArtifactModel.findOne({ gamePin: gamePin})
    if (allArtifacts.length === 0) {
      console.log('No  `${Artifact}` found.')
      const finalResult = { status: 'error', result: {}, message: `No ${Artifact} Exists` }
      return finalResult
    } else {
      const finalResult = {
        status: 'success',
...
export async function updateGameStatusWithCustomLog (gamePin, updatedData, updatedBy, customChangeMessage) {
  // Find the game by pin
  const gameStatus = await getByGamePin(gamePin)
  ...
  // Update game fields
  if (updatedData.gameStatus) {
    gameStatus.gameStatus = updatedData.gameStatus;
  }
  if (updatedData.endDateTime) {
    gameStatus.endDateTime = updatedData.endDateTime;
  }

  // Add user to audit field
  gameStatus.audit.updatedBy = updatedBy; // Who made the update

  // Push the custom change log message
  const changeDescription = `${customChangeMessage} (Updated by: ${updatedBy}) on ${new Date().toLocaleString()}`;
  gameStatus.audit.changeLog.push(changeDescription);

  try{
  // Save the updated game status
  var saveResult = await gameStatus.save();
  if (!saveResult) {
    console.error(`${Artifact}` + 'not updated successfully.')
    return { status: 'error', result: null, message: `${Artifact}` + 'not found for update.' }
  }
  }catch(e){
    console.error('Error getting active `${Artifact}`:', err)
...
// updateGameStatusWithCustomLog(
//   "654321", // gamePin
//   { gameStatus: "completed", endDateTime: new Date() }, // updatedData
//   "adminUser", // updatedBy
//   "Changed game status to 'completed' and added end time" // customChangeMessage
// );
