...
const JoinRequestScreen = ({ audience, removebutton }) => {
  ...
  // WebSocket connection for audience requests
  useEffect(() => {
    if (!audience?._id) return

    const wsUrl =
      typeof window !== 'undefined'
        ? `${window.location.protocol === 'https:' ? 'wss' : 'ws'}://${window.location.host}/api/ws/audience-requests/${
            audience._id
          }`
        : ''
    if (wsUrl) {
      ...
      wsRef.onopen = () => {
        console.log(`[WS] Connected to audience requests for audience ${audience._id}`)
        setIsConnected(true)
        setSocket(wsRef)
      }
      wsRef.onmessage = event => {
        try {
          ...
          if (msg.type === 'audienceRequests') {
            console.log('[WS] Audience requests updated:', msg.data)

            switch (msg.data.type) {
              case 'requestSent':
                console.log('New audience request received:', msg.data)
                if (msg.data.audienceId === audience?._id) {
              ...
              case 'requestApproved':
                console.log('Audience request approved:', msg.data)
                if (msg.data.audienceId === audience?._id) {
              ...
              case 'requestRejected':
                console.log('Audience request rejected:', msg.data)
                if (msg.data.audienceId === audience?._id) {
                  // Update the request status in the list
                  setRequests(prev =>
                    prev.map(req =>
          ...
          console.error('[WS] Error parsing audience requests message', e)
        }
      }
      wsRef.onerror = err => {
        console.error(`[WS] Audience requests error for audience ${audience._id}`, err)
        setIsConnected(false)
      }
      wsRef.onclose = () => {
        console.log(`[WS] Audience requests connection closed for audience ${audience._id}`)
        setIsConnected(false)
      }
  ...
  const fetchPendingRequests = async () => {
    ...
    setLoading(true)
    try {
      const result = await RestApi.get(`${API_URLS.v0.USERS_AUDIENCE_REQUEST}?audienceId=${audience._id}`)
  ...
  const handleApprove = async requestId => {
    ...
    try {
      const result = await RestApi.put(`${API_URLS.v0.USERS_AUDIENCE_REQUEST}`, {
        requestId,
        action: 'approve'
      })
