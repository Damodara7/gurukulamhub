...
const RoleDialog = ({ open, setOpen, title }) => {
  ...
  const handleCreateRole = async newRole => {
    try {
      // const result = await clientApi.addRole({
      //   ...newRole,
      //   createdBy: session?.user?.email
      // })
      const result = await RestApi.post(`${API_URLS.v0.ROLE}`, { ...newRole, createdBy: session?.user?.email })
    ...
    <Dialog fullWidth maxWidth='md' scroll='body' open={open} onClose={handleClose}>
      ...
      <form onSubmit={e => e.preventDefault()}>
        ...
        <DialogActions className='gap-2 justify-center pbs-0 pbe-10 pli-10 sm:pbe-16 sm:pli-16'>
          <Button variant='contained' type='submit' onClick={handleCreateRole}>
            Submit
          </Button>
