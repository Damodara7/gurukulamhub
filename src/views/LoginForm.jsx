...
const LoginForm = ({ mode, toggleAuthMode, customRedirectUrl = '/dashboards/myprogress' }) => {
  // Hooks
  ...
  const onSubmit = async data => {
    ...
    try {
      ...
      // Localized Redirect Url construction
      const redirectURL = searchParams.get('redirectTo') ?? customRedirectUrl
      console.log('RedirectURL: ', redirectURL)
      console.log('locale: ', locale)
      let localizedRedirectUrl = getLocalizedUrl(redirectURL, locale)
      ...
      if (loginMethod === 'email') {
        ...
        if (result?.error) {
          ...
          window.location.replace(localizedRedirectUrl)
  ...
  const onSubmitWithCode = async () => {
    try {
      ...
      const redirectURL = searchParams.get('redirectTo') ?? `/dashboards/myprogress`
      console.log('RedirectURL: ', redirectURL)
      console.log('locale: ', locale)
      let localizedRedirectUrl = getLocalizedUrl(redirectURL, locale)
      console.log('localizedRedirectUrl: ', localizedRedirectUrl)
  ...
  async function onSocialLogin(formData) {
    ...
    console.log(action)
    try {
      const result = await handleSocialLogin(formData)
      if (result?.error) {
        console.log('Social signin error : ', result?.error)
        throw new Error(result.error)
      }
  ...
  async function findAccountsWithMobile(mobileValue) {
    try {
      ...
      console.error(error)
      setErrorMsg('Failed to fetch accounts. Please try again.')
    } finally {
  ...
  return (
    <>
              ...
              {accountsWithMobile.length > 0 ? (
                ...
                loading.accountsFetched && (
                  ...
                  >
                    ...
                    <Typography style={{ cursor: 'pointer' }} onClick={toggleAuthMode} color='primary'>
                      Register?
                    </Typography>
                  </div>
                )
          ...
          <>
            {((otpSent && loginMethod === 'mobile') || loginMethod === 'email') && (
              <>
                ...
                <Button disabled={isSubmitting} fullWidth variant='contained' style={{ color: 'white' }} type='submit'>
      ...
      {showCode && (
        <form noValidate autoComplete='off' onSubmit={handleSubmit(onSubmitWithCode)} className='flex flex-col gap-4'>
          <CenterBox>
