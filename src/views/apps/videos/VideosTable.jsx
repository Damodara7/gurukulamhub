...
import VideoPortionPlayerDialog from '@/components/videos/VideoPortionPlayerDialog'
...
import { deleteVideo, getAllVideos } from '../../../actions/videos'
...
const ActionsMenu = ({ anchorEl, handleClose, handleAction }) => (
  <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleClose}>
    ...
    <MenuItem dense onClick={() => handleAction('play')}>
      <ListItemIcon>
        <PlayArrowIcon />
      </ListItemIcon>
...
const VideosTable = () => {
  ...
  const getVideosData = async () => {
    console.log('Fetching Videos Data now...')
    const result = await getAllVideos()
    if (result?.status === 'success') {
      console.log('Videos Fetched result', result)
      setData(result?.result || [])
    } else {
      console.log('Error:' + result?.message)
      console.log('Error Fetching Videos:', result)
    }
  }
  ...
  const refreshData = async () => {
    await getVideosData() // Fetch the latest Videos
  }
  ...
  const handleDelete = async () => {
    if (currentRow) {
      try {
        const result = await deleteVideo({ id: currentRow._id })
        if (result?.status === 'success') {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('url', {
        header: 'Video',
        cell: ({ row }) => <MediaPreviewPopup url={row.original.url} mediaType='video' showPopup={true} />
      }),
  ...
  const handlePlayRow = () => {
    setOpenPlayerDialog(true) // Open the dialog
  }
  ...
  const handleClosePlayerDialog = () => {
    setOpenPlayerDialog(false) // Open the dialog
    setCurrentRow(null)
    handleClose() // Close menu
  }
