...
import GeoRoleDialog from '@/components/dialogs/geo-role-dialog'
  ...
  const handleDelete = async () => {
    if (currentRole) {
      ...
      try {
        ...
        const result = await RestApi.del(`${API_URLS.v0.GEO_ROLE}?id=${currentRole._id}`)
        if (result?.status === 'success') {
